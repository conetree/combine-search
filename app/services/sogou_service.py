...
SOGOU_URL = config["SOGOU_URL"]
ALLOWED_DOMAIN = config["ALLOWED_DOMAIN"]

class SogouService(BaseSearch):
    ...
    def search(self, query: str, mode="text", links_num=2, headers: dict = None) -> Dict[str, Any]:
        print(f"SogouService->search({query})")
    ...
    def search_web(self, query: str, mode="text", links_num=2, headers: dict = None) -> dict:
        ...
        """
        search_url = f"{SOGOU_URL}?query={query}"
        try:
            # 1. 获取搜狗搜索结果页面
            headers = WebUtils.get_enhanced_headers(SOGOU_URL, headers)
            response = self.get_client().fetch(search_url, headers=headers)
            
            # 2. 使用 BeautifulSoup 解析返回页面，并提取搜索结果
            soup = BeautifulSoup(response, "html.parser")
            # 通过 class="r-sech" 查找结果
            result_items = soup.find_all(class_="r-sech")
            all_links = []
            for item in result_items:
                data_url = item.get("data-url")
                data_title = item.get("data-title")
                if data_url and data_title:
                    decoded_title = unquote(data_title)
                    all_links.append({"title": decoded_title, "href": data_url})
    ...
    def _extract_request_urls(self, urls, links_num=2):
        """
        提取请求链接，保证每个允许域名只取一个链接，若不足则补充其他链接
        """
        added_domains = set()
        result = []
        for item in urls:
