...
class BingService(BaseSearch):
    ...
    def search(self, query: str, mode="text", links_num=2, headers: dict = None)-> Dict[str, Any]:
        print(f"BingService->search({query})")
    ...
    def search_web(self, query: str, mode="text", links_num=2, headers: dict = None) -> dict:
        ...
        """
        search_url = f"{BING_URL}?q={query}"
        try:
            # 1. 获取 Bing 搜索结果页面
            # headers = headers or WebUtils.get_enhanced_headers(BING_URL)
            headers = WebUtils.get_enhanced_headers(BING_URL, headers)
            start_time = time.time()
            # 1. 获取 Bing 搜索结果页面
            logger.info(f"BingService search_web before self.get_client().fetch cost: {time.time() - start_time}")
            response = self.get_client().fetch(search_url, headers=headers)
            logger.info(f"BingService search_web after self.get_client().fetch cost: {time.time() - start_time}")

            # 2. 使用 BeautifulSoup 解析返回页面，并提取搜索结果
            soup = BeautifulSoup(response, "html.parser")
            # Bing 搜索结果通常以 <li class="b_algo"> 存在
            ...
            # 3. 过滤有效链接（仅保留指定域名中链接，可根据需求调整过滤规则）
            ...
            # logger.info("BingService 抓取链接:%s 抓取结果:%s", request_urls, contents_result)
    ...
    def extract_content_text_simple(self, html_content: str) -> str:
        """
        利用 BeautifulSoup 提取网页内可见文字，排除脚本、样式、head等内容
        """
